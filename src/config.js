// Editable configuration for the birthday site
// Update these values to instantly change the recipient and date

export const recipientName = "Samreen Ahatsham"; // Change the name here

// Set the birthday date as a human-readable string, e.g. "15 August 2025"
// You can also use ISO format "2025-08-15" if you prefer
export const birthdayDate = "13 February 2025"; // Change the date here

export const themeColors = {
  primary: "#ff6ec7", // candy pink
  secondary: "#ffc371", // sunset peach
  accent: "#a1c4fd", // soft blue
  glow: "#fff7b0",
  dark: "#0c0713",
};


